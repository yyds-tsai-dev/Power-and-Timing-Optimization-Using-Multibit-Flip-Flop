...
- ~~hc01 是 4-bit-first 的結構性論據~~ **撤回(1bis 後實測)**:hc01 base 佈局
  已 96% 4-bit 化(4,734×4b / 903×1b / 20×2b)——β=200,000 下前端 banking
  自己就把 4-bit 開滿了,殘餘候選池為**空**(quad 與 2b+2b 四個半徑全零)。
  R1 在 hc01 上行 ≈ 0;R1 真實標的收斂為 **hc02 + tc2**。R3 的 hc01 論據
  同步失效(我們的管線在此案已達成 NTU 式結果)。
...
- **候選池實測(tc2 base 佈局,server B 2026-07-11)**: 3,696 顆 2b MBFF 中,
  同時脈 + 兩 bit slack≥0 + Q 側 headroom ≥ dd·R + 距離 ≤ R 的貪婪配對:

  | R(距離) | 可行 4b 合併 | tc2 原始收益 | hc02 權重尺度 |
  |---|---|---|---|
  | 10,000 | 9 | 270 | 27,000 |
  | 20,000 | 69 | 2,070 | 207,000(hc02 的 2.1%) |
  | 50,000 | 162 | **4,860(NTU 差距的 29%)** | **486,000(hc02 的 4.8%)** |

  門檻是保守下界(headroom 要求整個 R 而實際各移 ~R/2;貪婪非最優;
  未計第二層 endpoint-slack 預算)。
- **hc02 原生實測(P1b binary,hc02 自己的 base 佈局,2026-07-11 追加)**:
  hc02 權重下前端已大量 4-bit 化(4,295 顆 4b;剩餘 2b 僅 1,977),真實池比
  tc2 外推小一個量級:R=20k → 10 對(+30,000);**R=50k → 37 對(+111,000 =
  官方 1.10%)**;R=100k → 43 對(1.28%)。外推值 4.8% 撤回;
  **1.1% 仍遠超 0.5% gate 門檻,R1 維持 Phase 2 高優先,期望值校正為 ~1% 級**。
  hc02 headroom 分佈比 tc2 肥(52% Q pins ≥10 TNS 單位,p50=18.1 vs 1.73),
  2g 在 hc02 的空間同步確認。
