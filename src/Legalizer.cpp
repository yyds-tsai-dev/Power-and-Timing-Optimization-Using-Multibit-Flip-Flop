...
size_t Legalizer::UpdateRowsFootprint(const Coor &coor, Cell *cell){
    Node *ff = new Node();
    ff->setGPCoor(coor);
    ff->setLGCoor(coor);
    ff->setCell(cell);
...
void Legalizer::PromoteFootprintToFF(size_t stubIdx, FF* newFF){
    Node *ff = ffs[stubIdx];
    ff->setName(newFF->getInstanceName());
    ff->setTNS(newFF->getTNS());
    ff->setFFPtr(newFF);
...
bool Legalizer::canPlaceFootprint(const Coor &coor, Cell *cell){
    size_t row_idx = FindClosestRow(coor);
    if(rows[row_idx]->getStartCoor().y != coor.y) return false;
    if(rows[row_idx]->hasCell(cell)) return false;
    return ContinousAndEmpty(coor.x, coor.y, cell->getW(), cell->getH(), (int)row_idx);
}
